...
TMP_DIR = Path(__file__).resolve().parent.joinpath("insightbot_data", "tmp")
LOCAL_VECTOR_STORE_DIR = (
    Path(__file__).resolve().parent.joinpath("insightbot_data", "vector_stores")
)

####################################################################
#            Create app interface with streamlit
####################################################################
st.set_page_config(page_title="Insightbot: Chat With Your Data")
...
def sidebar_and_documentChooser():
    """Create the sidebar and a tabbed pane: the first tab contains a document chooser (create a new Insightbot vectorstore);
    the second contains a vectorstore chooser (open an old Insightbot vectorstore)."""

    with st.sidebar:
        st.caption(
            "🚀 Insightbot: A Retrieval-Augmented Generation chatbot powered by 🔗 LangChain, Cohere, OpenAI, Google Generative AI, and 🤗 Hugging Face."
        )
        st.write("")
        ...
        st.write(
            ...
            and {st.session_state.retriever_type} are only considered when loading or creating an Insightbot vectorstore._"
        )

    # Tabbed Pane: Create a new Insightbot Vectorstore | Open a saved Insightbot Vectorstore

    tab_new_vectorstore, tab_open_vectorstore = st.tabs(
        ["Create a new Insightbot Vectorstore", "Open a saved Insightbot Vectorstore"]
    )
    with tab_new_vectorstore:
        ...
        st.session_state.vector_store_name = st.text_input(
            label="**Documents will be loaded, embedded and ingested into an Insightbot vectorstore (Chroma DB). Please provide a valid name.**",
            placeholder="Insightbot vectorstore name",
        )
        # 3. Add a button to process documents and create a Chroma vectorstore

        st.button("Create Insightbot Vectorstore", on_click=chain_RAG_blocks)
        try:
    ...
    with tab_open_vectorstore:
        # Open a saved Insightbot Vectorstore
        st.write("Please select an Insightbot Vectorstore:")
        import tkinter as tk
        from tkinter import filedialog

        clicked = st.button("Insightbot Vectorstore chooser")
        root = tk.Tk()
        root.withdraw()
        ...
        if clicked:
            ...
            # if API keys are inserted, start loading Chroma index, then create retriever and ConversationalRetrievalChain
                ...
                if selected_vectorstore_path == "":
                    ...
                    with st.spinner("Loading Insightbot vectorstore..."):
                        st.session_state.selected_vectorstore_name = (
                        ...
                        try:
                            ...
                            st.info(
                                f"Insightbot vectorstore **{st.session_state.selected_vectorstore_name}** loaded successfully."
                            )
...
def delte_temp_files():
    """delete files from the './insightbot_data/tmp' folder"""
...
def select_embeddings_model():
    """Select embeddings models: OpenAIEmbeddings, GoogleGenerativeAIEmbeddings, or HuggingFaceInferenceAPIEmbeddings."""
...
def create_retriever(
    ...
    """
    Create a retriever which can be:
        - Vectorstore backed retriever: this is the base retriever.
        - Contextual compression retriever: Wraps the base retriever in a ContextualCompressionRetriever.
...
def CohereRerank_retriever(
    ...
    """Build a ContextualCompressionRetriever using CohereRerank endpoint."""
...
def chain_RAG_blocks():
    """The Insightbot RAG system is composed of:
    - 1. Retrieval: document loaders, text splitter, vectorstore and retriever.
    - 2. Memory.
    - 3. Conversational Retrieval chain.
    """
    with st.spinner("Creating Insightbot vectorstore..."):
        # Check inputs
        error_messages = []
        if (
            not st.session_state.openai_api_key
        ...
        if st.session_state.vector_store_name == "":
            error_messages.append("provide an Insightbot vectorstore name")
            ...
            try:
                ...
                if st.session_state.uploaded_file_list is not None:
                    ...
                    try:
                        ...
                        st.info(
                            f"Insightbot vectorstore **{st.session_state.vector_store_name}** created successfully."
                        )
...
def answer_template(language="english"):
    """Pass the standalone question along with the chat history and context to the LLM which will answer."""
